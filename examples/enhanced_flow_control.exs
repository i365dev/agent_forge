 ...
 """
  ...
  defp run_skip_example do
    ...
    next_handler = fn _signal, state ->
      ...
      {{:emit, Signal.new(:processed, "processed data")}, Map.put(state, :next_handler_called, true)}
    end

    # Run with default settings (continue_on_skip = false)
    signal = Signal.new(:test, "test data")
    {result_type, _result, state} = Flow.process_with_limits([skip_handler, next_handler], signal, %{})
    ...
    {result_type, _result, state} = Flow.process_with_limits(
  ...
  defp run_signal_strategies do
      ...
      fn _signal, state ->
        ...
        case visits do
          ...
          1 ->
            ...
            {{:emit, Signal.new(:final, "final data")},
             Map.put(state, :visit_count, visits + 1)}
    ...
    {:ok, result, state} = Flow.process_with_limits(
      restart_handlers,
      Signal.new(:start, "start data"),
      %{},
      signal_strategy: :restart,
      max_steps: 10, # Explicit step limit as safety measure
      timeout_ms: 5000 # Short timeout as additional safety
    )
  ...
  defp run_branching_example do
    ...
    {:ok, result, _state} = Flow.process(branch_handlers, Signal.new(:branch, "long test string"), %{})
  ...
  defp run_function_flow do
    ...
    function_flow = fn signal, state ->
      ...
      case signal.type do
        ...
        :processing ->
          ...
          result = if String.length(signal.data) > 15 do
    ...
    # Run function flow
    {:ok, result, state} = Flow.process_function_flow(
      function_flow,
